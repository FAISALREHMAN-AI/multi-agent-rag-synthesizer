  ...
  const handleExecuteQuery = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setReportContent(`# Executive Knowledge Synthesis: ${query}\n\n` +
        `## 1. Executive Summary\n` +
        `This report presents a structured synthesis for **${activeProject.title}**, retrieved using **Hybrid Dense + Sparse BM25 RRF Search** and compiled via an autonomous **LangGraph Multi-Agent Workgroup**.\n\n` +
        `## 2. Multi-Agent Orchestration & Retrieval Architecture\n` +
        `- **Researcher Agent**: Scanned semantic vector spaces and keyword BM25 indices to extract high-density evidence chunks.\n` +
        `- **Writer Agent**: Transformed raw citations into publication-grade Markdown structures with clear section headings.\n` +
        `- **Reviewer Agent**: Executed automated self-correction cycles to verify factual integrity and prevent hallucination risks.\n\n` +
        `## 3. Key Findings & Insights\n` +
        `1. **Hybrid RRF Fusion**: Reciprocal Rank Fusion ($RRF$) successfully mitigates vocabulary mismatch while preserving vector semantic intent.\n` +
        `2. **Self-Correction Cycles**: Automated Reviewer feedback loops increase Ragas overall quality score to **91%**.\n` +
        `3. **Production Readiness**: Sub-second SSE streaming ensures continuous real-time progress visibility for end users.\n\n` +
        `---\n` +
        `*Generated by Synthetix AI Multi-Agent Synthesizer System.*`
      );
